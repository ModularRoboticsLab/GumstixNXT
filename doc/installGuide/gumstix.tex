...
Revision 1.4.0alpha (\today)
%
\begin{itemize}
\item From revision 1.3.2: added preliminary description of kernel
...
initializes the hardware.  Setting up the character device is done by
initializing the device structure, dynamically assigning the major
number, and requesting the kernel to create the entry under /dev.
Setting up the character device is not covered here.  Setting up the
hardware basically consists of requesting the required GPIOs and
configuring the directionality.

Response to user requests is implemented by the write function.  This
function examines the user input (found in the character buffer) to
decide what actions to take.  Actions are implemented by using the
appropriate GPIOs.  Care must be takes to return a value corresponding
to the number of bytes read.  Unloading is concerned with
...
of the motor.  (See the GumstixNXT schematics.)  Basic motor control
...
be read from the phase difference between TACHOxA and TACHOxB.

The simplest control is thus to set ENABLE to 1 and PHASE to whatever
direction you want, ignoring TACHO: this will make the motor rotate
...
the kernel.  Follow the guide on jumpnowtek.com, which basically
...
except ``Support for framebuffer devices''.

It is recommended that you configure the kernel and setup u-boot for
