...
\begin{itemize}
\item Revision 2.1 (\today): numerous minor fixes
\item Revision 2.0.2: cleaned up for new iteration of the course, enough checked and verified
\end{itemize}
...
This guide is assuming that the development host is running Ubuntu 10.04.04 LTS (all updates installed, see Appendix \ref{app:getting-linux} for a quick guide on getting Linux up and running).  VMWare or VirtualBox can is some cases be used (see Appendix \ref{app:virtualized} for some notes on using a virtualized host). We will also assume that the reader has access to the serial console port on the overo board, for instance by using GumstixNXT, the Tobi expansion board or similar.
...
OpenEmbedded (OE) is a build system used for the overo boards to
create Linux distributions. OE is used to build all the parts we need
for our Linux distribution including cross compilers, the kernel,
rootfs etc. First we will take a look at how to setup OE.\footnote{The
  instructions here are based on \url{http://gumstix.org/software-development/open-embedded/61-using-the-open-embedded-build-system.html}}
...
\promt{git checkout --track -b overo-2011.03 origin/overo-2011.03}
\end{quote}
...
\promt{git clone git://git.openembedded.org/bitbake bitbake}\\
\promt{cd bitbake}\\
\promt{git checkout 1.12.0}
\end{quote}
%openembedded.net er nede, desuden br github give bedre performance
...
This bitbake recipe will build a fairly small, but not minimal, Linux distribution. 
%
The distribution includes kernel modules for interfacing to the
wireless network, and software packages for various network-related
functionalities.
%
In order to build the distribution use the following command:
...
\noindent Here, after failing the initial HTTP request, the process continues using a different (albeit slower to obtain) source, and will normally proceed to the next task eventually.
...
The gumstix boards comes with Linux preinstalled, however, we
recommend using the SD card slot for booting the boards. The reason
for this is that if the contents of the onboard flash memory gets
corrupted due to an error during development it can be difficult to
reflash, however, an SD Card can easily be replaced.  
This means that if you power on the board without the SD card
inserted, it will boot using a standard Linux which will let you run
various commands, but does not provide dedicated drivers for the board.
...
The detailed instructions for how to modify the u-boot setup can be found on \url{jumpnowtek.com} under ``Gumstix u-boot dev''.
%
Note that the instructions cover two different versions, we are
working in the 2011.03 branch and must therefore work with ``u-boot'',
{\em not} ``u-boot-omap3''.
%
Following the guide on jumpnowtek.com, we 
...
patch to the recipe, and rebuild.  
%
The recipe to modify is \stt{org.openembedded.dev/recipes/u-boot/u-boot\_git.bb} and the \stt{SRC\_URI} to modify is
...
\noindent
% (Note: the description on the web page contains a few minor errors in the paths.)  
The same changes must
...
If you apply the changes incrementally, for example creating one patch
file for the first LED, another for the remaining LEDs, and yet
another for the motors (as described later), the end result can be
hard to understand and you can end up having conflicts between the
patches.  (In principle it should work fine, in practice it can cause trouble.)
...
\subsection{Cross-compilation environment}

To compile kernel modules, you must set up the shell environment to
enable access to the cross-compilers and other tools included with
Open Embedded.  The process is explained on
jumpnowtek.com\footnote{See for example the article
  \url{http://www.jumpnowtek.com/index.php?option=com_content&view=article&id=46&Itemid=54}},
and basically consists of sourcing (reading into the shell) the file
kernel-dev.txt, which can be done by the command

\begin{verbatim}
. kernel-dev.txt
\end{verbatim}

\noindent A slightly customized version of the file is available from GumstixNXT github.
...
%
Note that one of the TACHO signals is read from GPIO 31, which
according to the technical reference manual is bound to a register
named ``CONTROL\_PADCONF\_JTAG\_EMU0[31:16]'', this register however
cannot be found in the usual overo.h file.  The register definition
line can be added manually, to find the correct name use the find
command, e.g.:
%
\begin{quote}
\begin{verbatim}
...
\end{quote}
%
(This assumes you have defined UBOOTDIR like on jumpnowtek.com.)  This
reveals the name CONTROL\_PADCONF\_JTAG\_EMU1 defined in
./git/arch/arm/include/asm/arch-omap3/mux.h so the following line
should be added to overo.h:
%
\begin{quote}
\begin{verbatim}
...
the kernel.  Follow the ``Gumstix kernel dev'' guide on jumpnowtek.com, 
%
except that with the default bitbake setting the configuration menu needs 
to be accessed using the ``screen'' program, which it a bit cumbersome.
To fix this, edit
...
\noindent by replacing the two lines
...
\noindent with the following two lines (that instruct it to open a regular terminal window instead):
...
\noindent Once this is done, configuring the kernel basically
...
\noindent This gives you a textual menu for configuring the kernel.  
...
\begin{quote}
%cp ${OVEROTOP}/tmp/work/overo-angstrom-linux/gnueabi/linux-omap3-2.6.36-r97/git/.config ${OVEROTOP}/org.openembedded.dev/recipes/linux/linux-omap3-2.6.36/defconfig
\begin{verbatim}
cp tmp/work/overo-angstrom-linux-gnueabi/linux-omap3-3.2-r103/git/.config 
   org.openembedded.dev/recipes/linux/linux-omap3-2.6.37-psp04.01.00.03/defconfig
...
position of the motor and adjust the PWM control accordingly.  Apart
from the control algorithm, the main difficulty is that this should
happen at regular intervals, meaning it should be run by a timer
interrupt.  This will be covered in a future version of the guide.

\subsection{Developing a sensor controller}
...
also been shown to work, and 12.04 can be made to work with a bit of reconfiguration).  To get Ubuntu 10.04 LTS running on your
...
The third option is recommended for beginners: it requires you to have
...
to work with less: 40Gb of space seems to work, and if short on space 30Gb can be used as a 
minimum.\footnote{Wubi by default imposes af maximum size of 30Gb,
  this works for what you will normally be doing in this class.  If
  you run out of space, it is fairly easy to expand dynamically
  afterards by adding another virtual harddisk for storing the open
  embedded environment.  The Wubi main disk can also be expanded after
  installation, but this is out of the scope of this guide.}

To install Ubuntu 10.04.04 LTS (also called ``Lucid Lynx'') on your
machine, download the appropriate file from the corresponding
repository --- typically \url{http://releases.ubuntu.com/lucid/} ---
so to use Wubi, download \stt{wubi.exe} from this URL.  Linux
sometimes has issues with specific peripherals and hardware-specific
facilities, in particular on portable PCs.  Graphics cards and power
management are typical culprits, as is the wireless network.  Graphics
cards and power management typically work but with a reduced
functionality (in which case e.g.\ graphics will slow and the computer
cannot be put to sleep).  The wireless network is more of an issue,
but here an external USB network card can be used.  The ASUS USB-N13
is verified to work with Ubuntu 10.04.04, although unfortunately not
out-of-the-box, it requires using appropriate drivers which can be
downloaded and transferred via USB-stick:

\verb!http://driverscollection.com/?H=USB-N13%20%28VER.B1%29&By=ASUS!

\noindent (There is an error in the ``install.sh'' script, everywhere
it says ``su'' without ``sudo'', change it to ``sudo su''.)

Once the module has been installed, plug in the USB network adapter.
You can now connect to eduroam, but make sure to pick ``PEAP'' for
``EAP method'' (and don't worry about the lack of a certificate).
...
with Ubuntu (although as little as 30Gb can work, and 60Gb is
apparently recommended).  The hard part is interacting with the SD
card, which requires the ability to partition (MBR scheme) and create
filesystems (FAT and EXT3).  We have tried two different options:
attaching the card reader directly to the virtualized host, and using
Mac OS to interact with the SD card.
